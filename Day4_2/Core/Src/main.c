...
int main(void)
{
  ...
  if(xRet != pdTRUE)
	  Error_Handler();
  xRet = xTaskCreate(vLedTask1, "LED2", configMINIMAL_STACK_SIZE, &p2, 1, NULL);
  if(xRet != pdTRUE)
	  Error_Handler();
  xRet = xTaskCreate(vLedTask1, "LED3", configMINIMAL_STACK_SIZE, &p3, 1, NULL);
  if(xRet != pdTRUE)
	  Error_Handler();
  xRet = xTaskCreate(vLedTask1, "LED4", configMINIMAL_STACK_SIZE, &p4, 1, NULL);
  if(xRet != pdTRUE)
	  Error_Handler();
